...
/// Style typographique de la carte — les trois familles sont EMBARQUÉES
/// (cf. `pubspec.yaml`), jamais téléchargées.
///
/// Pas de `GoogleFonts.*` ici : ce serait une requête à fonts.gstatic.com au
/// premier rendu, et la police système de secours chez qui génère sa carte hors
/// réseau — donc une image de marque différente selon la connexion, ce que la
/// règle 1 ci-dessous interdit précisément.
///
/// Les familles sont variables : comme dans [AppText], l'axe `wght` doit être
/// renseigné en plus de `fontWeight`, qui seul ne l'instancie pas sur toutes
/// les plateformes.
TextStyle _face(
  String family, {
...
class ScoreShareCard extends StatelessWidget {
  ...
  static const double safeHeightPx =
      height * capturePixelRatio - 2 * unsafeBandPx;
  ...
  /// redistribue au lieu de déborder. Les métriques de police varient (réglage
  /// d'accessibilité système, police de repli si un asset venait à manquer)
  /// et une composition rigide finirait par rogner du
